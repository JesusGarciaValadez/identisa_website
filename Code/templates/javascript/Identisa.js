...
( function ( $, window, document, undefined ) {
    ...
    var _Identisa    = window._Identisa,
    // Use the correct document accordingly with window argument (sandbox)
    document    = window.document,
    location    = window.location,
    navigator   = window.navigator,
    // Map over Identisa in case of overwrite
    _Identisa    = window.Identisa;
    // Define a local copy of Identisa
    Identisa = function() {
        if ( !( this instanceof Identisa ) ) {
            // The Identisa object is actually just the init constructor 'enhanced'
            return new Identisa.fn.init();
        }
        return Identisa.fn.init();
    };
    ...
    //  Object prototyping
    Identisa.fn = Identisa.prototype = {
        /**
         *
        ...
        anchorMenu:             function ( selectorToApply, offsetTop, cssToFix, cssToDeFix ) {
            Identisa.tool = (window.pageYOffset !== undefined) ? window.pageYOffset : (document.documentElement || document.body.parentNode || document.body).scrollTop;
        ...
        closeAlert:             function ( ) {
            ...
            if ( typeof( Identisa.closer ) !== 'undefined' ){
                Identisa.overlay.close();
            } else {
        ...
        validateContactForm:   function ( dataPass ) {
            ...
            //  Valida el nombre
            if ( !Identisa._validateMinLength( 2, dataPass.nombre.length ) ) {
                Identisa.openAlert( 'Error', '<p>Por favor, escribe tu nombre.</p>' );
                return false;
            }

            //  Valida el apellido
            if ( !Identisa._validateMinLength( 2, dataPass.apellido.length ) ) {
                Identisa.openAlert( 'Error', '<p>Por favor, escribe tu apellido.</p>' );
                return false;
            }

            //  Valida el correo
            if ( !Identisa._validateMinLength( 2, dataPass.correo.length ) ) {
                Identisa.openAlert( 'Error', '<p>Por favor, escribe tu email.</p>' );
                return false;
            }
            if ( !Identisa._validateMail( dataPass.correo ) ) {
                Identisa.openAlert( 'Error', '<p>Por favor, escribe un email válido.</p>' );
                return false;
            }

            //  Valida el teléfono
            if ( !Identisa._validateMinLength( 7, dataPass.telefono.length ) ) {
                Identisa.openAlert( 'Error', '<p>El número no debe tener menos de 8 caracteres.</p>' );
                return false;
            }
            if ( !Identisa._validateMaxLength( 20, dataPass.telefono.length ) ) {
                Identisa.openAlert( 'Error', '<p>El número no debe tener mas de 20 caracteres.</p>' );
                return false;
            }
            if ( !Identisa._validateNumber( dataPass.telefono ) ) {
                Identisa.openAlert( 'Error', '<p>Por favor, escribe sólo números.</p>' );
                return false;
            }

            //  Valida que se escriba un mensaje
            if ( !Identisa._validateMinLength( 8, dataPass.mensaje.length ) ) {
                Identisa.openAlert( 'Error', '<p>Por favor, escribe tu mensaje para nosotros.</p>' );
                return false;
            }
            if ( !Identisa._validateMaxLength( 140, dataPass.mensaje.length ) ) {
                Identisa.openAlert( 'Error', '<p>Tu mensaje debe tener mas de 140 caracteres.</p>' );
                return false;
            }
            ...
            $.ajax ( 'Code/snippets/dispatcher.php?action=sendAppointment', {
                ...
                error:  function () {
                    ...
                    Identisa.openAlert( _title, _markup );
                },
    ...
    // Give the init function the Identisa prototype for later instantiation
    Identisa.fn.init.prototype = Identisa.fn;

    Identisa = Identisa.fn;

    // Expose Identisa to the global object
    window.Identisa  = Identisa;
